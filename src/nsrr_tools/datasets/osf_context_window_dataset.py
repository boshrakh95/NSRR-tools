#!/usr/bin/env python3
"""
osf_context_window_dataset.py — OSF baseline, Stage 1 Step 2

PyTorch Dataset that serves fixed-length context windows of OSF embeddings
for the context-length sweep experiments. Forked from
context_window_dataset.py (ContextWindowDataset) per
docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §3.2/Appendix §3.2 — the K-sampling
logic, SubjectGroupedSampler, and window-building index math are shape-
agnostic and copied as-is; only the embedding-shape constants change, and
the zero_modality_indices ablation feature is dropped entirely (OSF has no
4-modality-group structure to ablate the way SleepFM does).

CORE DESIGN PRINCIPLE (unchanged from ContextWindowDataset)
─────────────────────
...
differs. See context_window_dataset.py's docstring for the full
seq2seq/seq2label design rationale (identical here).

INPUT FILES
───────────
  {embedding_dir}/{dataset}/{subject_id}.npy
    dtype  : float16
    shape  : [T, 2, 768]   T = total 30-sec epochs for the full night
                           2 = [CLS token, mean-pooled patch tokens]
...
──────────────────────
  x    : float32 tensor  [N, 1536]  N = context_epochs, 1536 = 2×768
  mask : bool tensor     [N]        True = padded position (no real signal)
  y    : int64 tensor    []         scalar class label

CONTEXT LENGTH STRINGS — recomputed for 30-second epochs (was 5-second
patches in ContextWindowDataset; same string format, different patch
count per string since PATCH_SECONDS=30 here):
  "30s"        →    1 epoch
  "10m"        →   20 epochs
  "40m"        →   80 epochs
  "80m"        →  160 epochs
...
Since OSF's embedding granularity is already one row per 30-second AASM
epoch (unlike SleepFM's 5-second sub-epoch patches), PATCHES_PER_EPOCH=1
here (was 6 in ContextWindowDataset) — an "epoch" and a "patch" are the
same unit for OSF. This resolves the plan doc's previously-open question
about what PATCHES_PER_EPOCH should become for OSF.

MINIMUM PAST CONTEXT (seq2seq only) — same formula as ContextWindowDataset,
recomputed for 30s units:
    min_past = max(PATCHES_PER_EPOCH, min(N // min_past_denom, max_min_past))
  max_min_past_patches must represent the same wall-clock cap (20 min) in
  OSF's units: 20*60/30 = 40, not ContextWindowDataset's 240 (which was
  240*5s = 20min in SleepFM's units). Set in configs/phase0_osf_config.yaml.
"""
...
# ── Constants ─────────────────────────────────────────────────────────────────
PATCH_SECONDS     = 30          # each OSF embedding row = one 30-second epoch
PATCHES_PER_EPOCH = 1           # 30-sec sleep epoch / 30-sec OSF embedding — same unit
REM_ORIGINAL      = 5
REM_REMAPPED      = 4
EMBED_DIM         = 768
N_SUBTOKENS       = 2           # index 0 = CLS, index 1 = mean-pooled patches
FLAT_DIM          = N_SUBTOKENS * EMBED_DIM   # 1536
...
def _compute_min_past(N: int, denom: int = 8, max_patches: int = 40) -> int:
    ...
    Default max_patches=40 represents 20 minutes at 30s/epoch (was 240 at
    5s/patch in ContextWindowDataset) — see module docstring.
    """
...
def _load_emb_shape(path: Path) -> int:
    """Return T (number of epochs) without loading the full array."""
...
class OSFContextWindowDataset(Dataset):
    """Fixed-length (or full-night) context-window dataset over OSF embeddings.

    Args:
        cfg            : Phase 0 OSF config dict (from phase0_osf_config.yaml).
    ...
    def __init__(
        ...
        # min_recording_patches must be in OSF's 30s-epoch units (480 for
        # 240m, not ContextWindowDataset's 2880 5s-patch value) — set in
        # configs/phase0_osf_config.yaml. See docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §3.2.
    ...
    # ── Index builders ─────────────────────────────────────────────────────
    # Identical logic to ContextWindowDataset — purely integer index-arithmetic
    # on T (from shape cache) and N (from parse_context_length), no reference
    # to embedding shape constants. Copied as-is.
    ...
    def _build_seq2label_index(self) -> List[Tuple[int, int, int]]:
        ...
        for row_idx, row in self.df.iterrows():
            ...
            if self.split == "train":
                ...
                # K=K_max is achievable at all context lengths.
                n_valid = T - N + 1
                K = min(self._K_max, n_valid)
    ...
    def __getitem__(self, idx: int):
        ...
        if getattr(self, "_cached_path", None) != path_str:
            ...
            self._cached_emb  = np.load(npy_path, mmap_mode="r")  # (T, 2, 768) float16
    ...
    def _get_causal_window(
        ...
        w = window.astype(np.float32)      # (N, 2, 768) — always a copy, safe to mutate
    ...
    def _get_seq2label_window(
        ...
        if end <= T:
            window = emb[window_start : end]   # (N, 2, 768)
            mask   = np.zeros(N, dtype=bool)
        else:
    ...
    def collate_fn(batch):
        ...
        Use as: DataLoader(..., collate_fn=OSFContextWindowDataset.collate_fn)
    ...
    # subsets (e.g. --limit 2 may only contain one class). Tier-1 tasks
    # (docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §0) added explicitly; the rest
    # carried over from ContextWindowDataset for Tier-2/future parity.
    _TASK_NUM_CLASSES = {
        "sex_binary":               2,
        "sleep_efficiency_binary":  2,
        "bmi_binary":               2,
        "age_class":                3,   # verified against task_subjects CSV, 2026-08-11
        "apnea_binary":             2,
...
class SubjectGroupedSampler(torch.utils.data.Sampler):
    ...
    Identical to context_window_dataset.py's SubjectGroupedSampler — purely
    index-arithmetic, no reference to embedding shape. Copied as-is (kept as
    its own class here, not imported, so this dataset module has no import
    dependency on the SleepFM-side file).
