...
# Mantis baseline — Stage 1 Step 9 — Subject-level inference (all windows)
#
# Forked from jobs/infer_physioomni_subject_windows_gpu.sh (itself forked
# from infer_osf_subject_windows_gpu.sh) — see
...
# job scripts; only the venv, worktree path, target script, log dir, and
# job name differ (plus dropped ZERO_MODALITIES, which doesn't apply to
# Mantis either — no 4-modality-group structure to ablate).
#
...
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_mantis_subject_windows_gpu.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_mantis_subject_windows_gpu.sh
#
...
echo "========================================================================"
echo "Mantis baseline — Subject-level inference (all windows)"
echo "========================================================================"
