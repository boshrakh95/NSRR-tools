...
# windows) (rorqual-specific)
#
# Identical to infer_mantis_lora_subject_windows_gpu.sh except:
...
# TRAINING (jobs/train_mantis_lora_gpu_rorqual.sh, which needs the whole
# H100 for backward-pass activation memory), inference runs entirely under
# torch.no_grad() — no backward pass, no gradient checkpointing concern,
# same "MIG is fine, no memory argument for a whole card" logic that
# already applies to Stage 1 extraction.
#
...
# 5 min).
# Auto-resume on timeout: same mechanism as train_mantis_lora_gpu_rorqual.sh —
...
#       jobs/infer_mantis_lora_subject_windows_gpu_rorqual.sh
#
...
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_mantis_lora_subject_windows_gpu_rorqual.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_mantis_lora_subject_windows_gpu_rorqual.sh
#
...
# Unbuffered stdout — see jobs/train_mantis_lora_gpu_rorqual.sh for why this matters.
...
echo "========================================================================"
echo "Mantis baseline — Stage 2 (LoRA) — Subject-level inference (all windows) (rorqual)"
echo "========================================================================"
