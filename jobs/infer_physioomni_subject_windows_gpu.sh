#!/bin/bash
#SBATCH --job-name=physioomni_infer_windows
...
# PhysioOmni baseline — Phase 1 Step 7 — Subject-level inference (all windows)
#
# Forked from jobs/infer_osf_subject_windows_gpu.sh — see
# docs/TSFM_PHYSIOOMNI_IMPLEMENTATION_PLAN.md §11/§13 (Appendix). Same
# structure, same auto-resume mechanism, same status/log conventions as
# OSF's job script; only the venv, target script, log dir, and job name
# differ (plus dropped ZERO_MODALITIES, which doesn't apply to PhysioOmni
# either).
#
...
# Auto-resume on timeout: same mechanism as train_physioomni_context_sweep_gpu.sh —
...
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_physioomni_subject_windows_gpu.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_physioomni_subject_windows_gpu.sh
#
# Time guide: NOT YET CALIBRATED for PhysioOmni (same open caveat as OSF's
# own script) — start with generous --time estimates for the first few runs.
...
echo "========================================================================"
echo "PhysioOmni baseline — Subject-level inference (all windows)"
echo "========================================================================"
...
# ── Build command ─────────────────────────────────────────────────────────────
CMD="python scripts/infer_physioomni_subject_windows.py --config $CONFIG"
