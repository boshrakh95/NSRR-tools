...
#
# Forked from jobs/train_context_sweep_gpu.sh — see
# docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §3.6 (Appendix). Same structure,
# same auto-resume mechanism, same status/log conventions as the SleepFM
# job script; only the venv, target script, log dir, and job name differ.
#
...
#   invocation by gen_commands_osf.py, not inside this script — see plan §3.6).
#
# Usage examples:
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=lstm        jobs/train_osf_context_sweep_gpu.sh
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=mean_pool    jobs/train_osf_context_sweep_gpu.sh
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=transformer  jobs/train_osf_context_sweep_gpu.sh
#
#   # Filter to specific datasets (space-separated, quoted):
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=lstm,DATASETS="apples shhs" jobs/train_osf_context_sweep_gpu.sh
#
#   # Disable W&B for a run:
#   sbatch --export=ALL,...,NO_WANDB=1 jobs/train_osf_context_sweep_gpu.sh
#
# W&B setup: store your API key in ~/.wandb_key (chmod 600).
#   The script loads it automatically — no interactive prompts.
#   OSF runs default to the nsrr-phase0-osf W&B project (kept separate from
#   SleepFM's nsrr-phase0), see train_osf_context_sweep.py.
#
# Or single default run (uses task/head from phase0_osf_config.yaml):
#   sbatch jobs/train_osf_context_sweep_gpu.sh
...
WANDB_PROJECT=${WANDB_PROJECT:-"nsrr-phase0-osf"}
NO_WANDB=${NO_WANDB:-""}
...
echo "========================================================================"
echo "OSF baseline — Stage 1 Step 4 — Context-Length Sweep"
echo "========================================================================"
...
# ── Build command ─────────────────────────────────────────────────────────────
CMD="python scripts/train_osf_context_sweep.py --config $CONFIG"
