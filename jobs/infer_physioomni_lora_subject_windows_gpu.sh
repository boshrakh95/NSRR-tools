...
#SBATCH --output=/home/boshra95/NSRR-tools-main/logs_physioomni_lora/%x_%j.out
#SBATCH --error=/home/boshra95/NSRR-tools-main/logs_physioomni_lora/%x_%j.err

# PhysioOmni baseline — Stage 2 Step 2 — LoRA Subject-level inference (all windows)
#
# Forked from jobs/infer_osf_lora_subject_windows_gpu.sh — see
...
# training's job scripts; only the venv (physioomni_env), target script,
# config, log dir, and job name differ. No TASK_TYPE here — Stage 2 is
# seq2label-only (physioomni_raw_epoch_dataset.py scope note).
#
# Loads a train_physioomni_lora.py checkpoint (peft state dict: LoRA
# deltas + sequence_head) and runs the LoRA-adapted encoders live on raw
# signal for ALL non-overlapping windows per subject (no K=5 cap). Saves a
# parquet of per-window probabilities for downstream majority-voting /
# mean-prob aggregation — same schema as Stage 1's inference output.
#
...
# incrementally resumable (see infer_physioomni_lora_subject_windows.py's
# RESUMABILITY docstring note) — a timeout picks up from the last periodic
# checkpoint (every 5 min), not item 0.
# Auto-resume on timeout: same mechanism as train_physioomni_lora_gpu.sh —
...
# Usage examples:
#   # Single context:
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm,CONTEXTS="10m" \
...
#
#   # With dataset filter:
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm,CONTEXTS="30s 10m",DATASETS="apples shhs" \
#       jobs/infer_physioomni_lora_subject_windows_gpu.sh
#
#   # Run on val split instead of test:
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_physioomni_lora_subject_windows_gpu.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_physioomni_lora_subject_windows_gpu.sh
#
# ⚠️ WALL-TIME NOT CALIBRATED — 5h default copied from Stage 1's inference
# job as a placeholder. Stage 2 inference is more expensive per window
# than Stage 1's (live LoRA-adapted encoder forward passes per raw epoch,
# not a cached-embedding lookup) — revisit after the first real pilot.
...
echo "========================================================================"
echo "PhysioOmni baseline — Stage 2 (LoRA) — Subject-level inference (all windows)"
echo "========================================================================"
...
# ── Build command ─────────────────────────────────────────────────────────────
CMD="python scripts/infer_physioomni_lora_subject_windows.py --config $CONFIG"
