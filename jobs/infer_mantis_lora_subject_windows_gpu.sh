...
# windows) (checklist 2.5)
#
# Forked from jobs/infer_osf_lora_subject_windows_gpu.sh — see
# docs/TSFM_MANTIS_IMPLEMENTATION_PLAN.md §14. Same structure, same
# auto-resume mechanism, same status/log conventions as Stage 1's/Stage 2
# training's job scripts; only the venv, target script, config, log dir,
# and job name differ. No TASK_TYPE here — Stage 2 is seq2label-only.
#
# GPU size: **MIG slice (1g.10gb), NOT the whole card** — unlike Stage 2
# TRAINING (jobs/train_mantis_lora_gpu.sh, which needs the whole H100 for
# backward-pass activation memory, plan §4.5), inference runs entirely
# under torch.no_grad() — no backward pass, no gradient checkpointing
# concern, same "MIG is fine, no memory argument for a whole card" logic
# that already applies to Stage 1 extraction.
#
...
# incrementally resumable (see infer_mantis_lora_subject_windows.py's
# RESUMABILITY docstring note) — a timeout does not restart an "all
# windows" pass from item 0; it picks up from the last periodic checkpoint
# (every 5 min). Verified live in checklist 2.5's own smoke test: a
# 2-subject/2,084-item CPU run correctly checkpointed at 72% progress
# mid-run.
# Auto-resume on timeout: same mechanism as train_mantis_lora_gpu.sh —
...
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_mantis_lora_subject_windows_gpu.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_mantis_lora_subject_windows_gpu.sh
#
...
# Unbuffered stdout — see jobs/train_mantis_lora_gpu.sh for why this matters.
...
echo "========================================================================"
echo "Mantis baseline — Stage 2 (LoRA) — Subject-level inference (all windows)"
echo "========================================================================"
