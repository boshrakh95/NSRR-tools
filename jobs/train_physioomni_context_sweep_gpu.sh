#!/bin/bash
#SBATCH --job-name=physioomni_ctx_sweep
...
# differ. Runs from THIS worktree (/home/boshra95/NSRR-tools-omni,
# physioomni-implementation branch) — never touches /home/boshra95/NSRR-tools
# (see plan §1's worktree isolation rule).
#
...
#   invocation, not inside this script — same convention as OSF's job script).
#
# Usage examples:
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm        jobs/train_physioomni_context_sweep_gpu.sh
#   sbatch --export=ALL,TASK=sex_binary,HEAD=mean_pool    jobs/train_physioomni_context_sweep_gpu.sh
#   sbatch --export=ALL,TASK=sex_binary,HEAD=transformer  jobs/train_physioomni_context_sweep_gpu.sh
#
#   # Filter to specific datasets (space-separated, quoted):
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm,DATASETS="apples shhs" jobs/train_physioomni_context_sweep_gpu.sh
#
#   # Disable W&B for a run:
#   sbatch --export=ALL,...,NO_WANDB=1 jobs/train_physioomni_context_sweep_gpu.sh
#
# W&B setup: store your API key in ~/.wandb_key (chmod 600).
#   The script loads it automatically — no interactive prompts.
#   PhysioOmni runs default to the nsrr-phase0-physioomni W&B project (kept
#   separate from SleepFM's nsrr-phase0 and OSF's nsrr-phase0-osf), see
#   train_physioomni_context_sweep.py. NOTE: wandb is not installed in
#   physioomni_env (same known CC Go-toolchain issue OSF hit) — use
#   NO_WANDB=1 until/unless that's resolved.
#
# Or single default run (uses task/head from phase0_physioomni_config.yaml):
#   sbatch jobs/train_physioomni_context_sweep_gpu.sh
...
WANDB_PROJECT=${WANDB_PROJECT:-"nsrr-phase0-physioomni"}
NO_WANDB=${NO_WANDB:-""}
...
echo "========================================================================"
echo "PhysioOmni baseline — Stage 1 Step 4 — Context-Length Sweep"
echo "========================================================================"
...
# ── Build command ─────────────────────────────────────────────────────────────
CMD="python scripts/train_physioomni_context_sweep.py --config $CONFIG"
