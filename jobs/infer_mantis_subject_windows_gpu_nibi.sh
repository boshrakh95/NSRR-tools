#!/bin/bash
#SBATCH --job-name=mantis_infer_windows
...
# Mantis baseline — Stage 1 Step 9 — Subject-level inference (all windows) (nibi-specific)
#
# Differs from infer_mantis_subject_windows_gpu.sh (Fir) the same way
# train_mantis_context_sweep_gpu_nibi.sh differs from its Fir counterpart —
# see that script's header for the full reasoning (whole-GPU h100:1 instead
# of a MIG slice, no --partition, no --exclude). Run
# jobs/test_gpu_setup_nibi.sh first if you haven't already.
#
...
# Usage examples:
#   # Single context:
#   sbatch --export=ALL,TASK=sex_binary,TASK_TYPE=seq2label,HEAD=lstm,CONTEXTS="10m" \
#       jobs/infer_mantis_subject_windows_gpu_nibi.sh
#
...
#
#   # With dataset filter:
#   sbatch --export=ALL,TASK=sex_binary,TASK_TYPE=seq2label,HEAD=lstm,CONTEXTS="30s 10m",DATASETS="apples shhs" \
#       jobs/infer_mantis_subject_windows_gpu_nibi.sh
#
#   # Point at the Pilot 1/2 population instead of the production
#   # embedding_dir (before checklist 1.11's full extraction has run):
#   sbatch --export=ALL,...,EMBEDDING_DIR=/scratch/boshra95/psg/unified/embeddings/mantis_pilot12/D_Llast_combined \
#       jobs/infer_mantis_subject_windows_gpu_nibi.sh
#
#   # Run on val split instead of test:
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_mantis_subject_windows_gpu_nibi.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_mantis_subject_windows_gpu_nibi.sh
#
# Time guide: NOT YET CALIBRATED for Mantis (same open caveat as OSF's/
# PhysioOmni's own scripts) — start with generous --time estimates for the
# first few runs.
...
echo "========================================================================"
echo "Mantis baseline — Subject-level inference (all windows) (nibi)"
echo "========================================================================"
...
# ── Build command ─────────────────────────────────────────────────────────────
CMD="python scripts/infer_mantis_subject_windows.py --config $CONFIG"
