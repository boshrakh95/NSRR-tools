#!/bin/bash
#SBATCH --job-name=osf_infer_windows
...
# OSF baseline — Stage 1 Step 5 — Subject-level inference (all windows)
#
# Forked from jobs/infer_subject_windows_gpu.sh — see
# docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §3.3/§3.6 (Appendix). Same
# structure, same auto-resume mechanism, same status/log conventions as
# the SleepFM job script; only the venv, target script, log dir, and job
# name differ (plus dropped ZERO_MODALITIES, which doesn't apply to OSF).
#
...
#
#   # With dataset filter:
#   sbatch --export=ALL,TASK=apnea_binary,TASK_TYPE=seq2label,HEAD=lstm,CONTEXTS="30s 10m",DATASETS="apples shhs" \
#       jobs/infer_osf_subject_windows_gpu.sh
#
#   # Run on val split instead of test:
#   sbatch --export=ALL,...,SPLIT=val jobs/infer_osf_subject_windows_gpu.sh
#
#   # Reproduce training eval exactly (K=5 windows, no --all-windows):
#   sbatch --export=ALL,...,NO_ALL_WINDOWS=1 jobs/infer_osf_subject_windows_gpu.sh
#
# Time guide: NOT YET CALIBRATED for OSF (unlike SleepFM's, which has
# observed-runtime numbers in the parent script's comment) — OSF's
# per-window cost profile differs (1536-dim embeddings vs 512-dim, 30s
# epochs vs 5s patches). Start with generous --time estimates for the
# first few runs; see docs/TSFM_OSF_IMPLEMENTATION_PLAN.md Appendix §3.5.
...
echo "========================================================================"
echo "OSF baseline — Subject-level inference (all windows)"
echo "========================================================================"
...
# ── Build command ─────────────────────────────────────────────────────────────
CMD="python scripts/infer_osf_subject_windows.py --config $CONFIG"
