...
# Mantis baseline — Stage 1 Step 8 — Context-Length Sweep Training (rorqual-specific)
#
# Identical to train_mantis_context_sweep_gpu.sh except:
#   - --partition=gpubase_bygpu_b3 (required on rorqual; not valid on Fir)
#   - --exclude removed (Fir fc* node names don't exist on rorqual)
#   - WANDB_MODE=offline (rorqual compute nodes have no internet; sync from
#     the login node afterwards with `wandb sync $WANDB_DIR`)
#
...
#
# Usage examples:
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm        jobs/train_mantis_context_sweep_gpu_rorqual.sh
#   sbatch --export=ALL,TASK=sex_binary,HEAD=mean_pool    jobs/train_mantis_context_sweep_gpu_rorqual.sh
#   sbatch --export=ALL,TASK=sex_binary,HEAD=transformer  jobs/train_mantis_context_sweep_gpu_rorqual.sh
#
#   # Filter to specific datasets (space-separated, quoted):
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm,DATASETS="apples shhs" jobs/train_mantis_context_sweep_gpu_rorqual.sh
#
...
#   sbatch --export=ALL,TASK=sex_binary,HEAD=lstm,EMBEDDING_DIR=/scratch/boshra95/psg/unified/embeddings/mantis_pilot12/D_Llast_combined jobs/train_mantis_context_sweep_gpu_rorqual.sh
#
#   # Disable W&B for a run:
#   sbatch --export=ALL,...,NO_WANDB=1 jobs/train_mantis_context_sweep_gpu_rorqual.sh
#
# W&B setup: store your API key in ~/.wandb_key (chmod 600).
#   The script loads it automatically — no interactive prompts. Runs offline
#   here (see above) — sync later with `wandb sync $WANDB_DIR` from the login
#   node once online.
...
#   sbatch jobs/train_mantis_context_sweep_gpu_rorqual.sh
...
export WANDB_MODE=offline                      # compute nodes have no internet; sync later from login node
...
echo "========================================================================"
echo "Mantis baseline — Stage 1 Step 8 — Context-Length Sweep (rorqual)"
echo "========================================================================"
...
echo "W&B:       $([ -n "$NO_WANDB" ] && echo disabled || echo "project=$WANDB_PROJECT (offline)")"
echo "Start:     $(date)"
echo "========================================================================"
echo ""
