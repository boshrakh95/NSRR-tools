...
# Phase 0 Step 4 — Context-Length Sweep Training (rorqual-specific)
#
# Identical to train_context_sweep_gpu.sh except:
#   - --partition=gpubase_bygpu_b3 (required on rorqual; not valid on Fir)
#   - --exclude removed (Fir fc* node names don't exist on rorqual)
#
# Usage examples:
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=lstm        jobs/train_context_sweep_gpu_rorqual.sh
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=mean_pool   jobs/train_context_sweep_gpu_rorqual.sh
#   sbatch --export=ALL,TASK=apnea_binary,HEAD=transformer jobs/train_context_sweep_gpu_rorqual.sh
#   sbatch --export=ALL,TASK=sleep_staging,HEAD=lstm       jobs/train_context_sweep_gpu_rorqual.sh
#
#   # Filter to specific datasets (space-separated, quoted):
#   sbatch --export=ALL,TASK=sleep_staging,HEAD=lstm,DATASETS="shhs1 shhs2" jobs/train_context_sweep_gpu_rorqual.sh
#
#   # Disable W&B for a run:
#   sbatch --export=ALL,...,NO_WANDB=1 jobs/train_context_sweep_gpu_rorqual.sh
#
