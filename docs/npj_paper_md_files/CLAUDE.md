...
`npj_supplementary.tex` yet. **2026-09-17 update: that draft doc now also has
Table~1/Table~3-style full-sweep and head-comparison tables for all three
baselines (Section 6, generated by `NSRR-tools/scripts/gen_tsfm_baseline_tables.py`,
not hand-typed), a consolidated cross-model table per head with all four
encoders and all six contexts together for easy row-by-row comparison
(Section 6.7, `--cross-model` flag on the same script), and two exploratory,
...
reversed a qualitative claim. This required one new, purely additive function
in `notebooks_npj/utils/data.py` (`load_heatmap_from_collected`) since the
three baselines don't have the
`final_results/.../heatmap_df_test.csv` tree the real paper figures read from
— see the draft doc's Section 7 for why and how it was verified.**
