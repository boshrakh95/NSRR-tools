#!/usr/bin/env python3
"""
Experiment command generator for PhysioOmni baseline experiments (v2_physioomni_registry.yaml).

Forked from gen_commands_osf.py — see
docs/TSFM_PHYSIOOMNI_IMPLEMENTATION_PLAN.md checklist item 1.8. Same core
pipeline, status tracking, and command-generation approach as the
SleepFM/OSF generators; a separate script rather than a gen_commands.py
retrofit per the "Code reuse assessment" decision in CLAUDE.md (no
backbone hook in the original registry schema/wall-time tables).
Differences from gen_commands_osf.py, all deliberate:
  - Targets experiments/v2_physioomni_registry.yaml, not
    v2_osf_registry.yaml — 4 Tier-1 tasks (sex, sleep efficiency, BMI, age),
    not 5: apnea_binary is excluded (no respiratory pathway in PhysioOmni,
    see CLAUDE.md's PhysioOmni section).
  - Targets jobs/{train,infer}_physioomni_*_gpu.sh (Fir only — no rorqual
    variant exists yet for PhysioOmni, same as OSF).
  - No --zero-modalities / ZERO_MODALITIES support (PhysioOmni has no
    4-modality-group structure to ablate — dropped from
    train_physioomni_context_sweep.py / infer_physioomni_subject_windows.py
    themselves, same as OSF's own fork).
  - python_bin fallback defaults to physioomni_env, not osf_env.
  - Figure/table subcommands (iso-plots, saturation, scaling-laws,
    ...
    are NOT ported — same reasoning as gen_commands_osf.py: SleepFM's own
    plot_*.py/make_table*.py scripts are already superseded by notebooks
    ...
    path here either. Once PhysioOmni results exist, feed them into
    notebooks the same way SleepFM's/OSF's are fed in.
  - Wall-time lookup tables are placeholder copies of OSF's own
    (themselves a placeholder copy of SleepFM's) — NOT YET CALIBRATED for
    PhysioOmni, no real GPU sweep has run yet (only CPU-debugged with
    --batch-size 2 as of 2026-08-18). Revisit after the first real
    PhysioOmni GPU sweep (checklist item 1.11); until then, generated
...
── Core pipeline ─────────────────────────────────────────────────────────────
  python scripts/gen_commands_physioomni.py list [--tier 1|2]
      List all PhysioOmni experiments with status (pending/trained/inferred/analyzed).

  python scripts/gen_commands_physioomni.py probe-batch <exp_id> [--starting-batch-size 256]
      Print sbatch command to probe the max GPU batch size (memory_bounded
      experiments only). No PhysioOmni experiment currently uses batch_mode:
      memory_bounded, and jobs/find_batch_size_physioomni_gpu.sh does not exist yet
      — this subcommand is kept for schema parity / future use only.

  python scripts/gen_commands_physioomni.py train <exp_id> [--context 30s 10m ...]
      Print sbatch command(s) for training. One job per context.

  python scripts/gen_commands_physioomni.py infer <exp_id> [--split test|val]
      Print the sbatch command for inference (auto-discovers trained contexts).

  python scripts/gen_commands_physioomni.py analyze <exp_id> [--plot] [--k-dense] [--bootstrap N]
      Print the python command for window analysis (analyze_windows.py —
      backbone-agnostic, reused unmodified; reads the PhysioOmni results dir).

  python scripts/gen_commands_physioomni.py build-heatmap <exp_id> [--split test]
      Print the build_heatmap_df.py command (iso-compute heatmap DataFrame;
      reused unmodified). Run analyze --k-dense first.

  python scripts/gen_commands_physioomni.py collect [<exp_id> ...]
      Print collect_results_v2.py command to gather all PhysioOmni results
      into training.csv and analysis.csv (reused unmodified).

  python scripts/gen_commands_physioomni.py threshold-tuning <exp_id>
      Print apply_threshold_tuning.py command for a binary experiment
      (reused unmodified).

  python scripts/gen_commands_physioomni.py status [<exp_id>]
      Show detailed file-level status for one or all experiments.

  python scripts/gen_commands_physioomni.py runs [<exp_id>]
      Show job run history (from logs_physioomni/status/*.jsonl tracking files).

Examples:
  python scripts/gen_commands_physioomni.py list --tier 1
  python scripts/gen_commands_physioomni.py train sex_binary_lstm
  python scripts/gen_commands_physioomni.py train sex_binary_lstm --context 30s
  python scripts/gen_commands_physioomni.py infer sex_binary_lstm
  python scripts/gen_commands_physioomni.py analyze sex_binary_lstm --plot
  python scripts/gen_commands_physioomni.py collect sex_binary_lstm bmi_binary_transformer
  python scripts/gen_commands_physioomni.py status
...
REGISTRY_PATH = Path(__file__).parent.parent / "experiments" / "v2_physioomni_registry.yaml"
JOBS_DIR = Path(__file__).parent.parent / "jobs"

_TRAIN_SCRIPT = "train_physioomni_context_sweep_gpu.sh"
_INFER_SCRIPT = "infer_physioomni_subject_windows_gpu.sh"
_PROBE_BATCH_SCRIPT = "find_batch_size_physioomni_gpu.sh"  # NOT YET IMPLEMENTED — see probe-batch docstring above

# ── Wall-time lookup tables ────────────────────────────────────────────────────
# NOT YET CALIBRATED FOR PHYSIOOMNI — placeholder copy of OSF's own tables
# (scripts/gen_commands_osf.py), themselves a placeholder copy of SleepFM's
# (calibrated from sex_binary_lstm on H100). PhysioOmni's per-epoch cost
# profile (500-dim flattened embeddings vs OSF's 1536-dim, same 30s-epoch
# token unit) has not been measured on GPU as of 2026-08-18 — only
# CPU-debugged with --batch-size 2. Revisit these numbers after the first
# real GPU sweep (checklist item 1.11). Auto-requeue on timeout means an
# underestimate just triggers one extra job — no training state is lost
# (resume.pt is saved after every epoch).
_TRAIN_HOURS = {
...
def _cfg_arch_tag(exp: dict, registry: dict) -> str:
    ...
    No PhysioOmni registry entry is seq2seq yet (sleep_staging is deferred,
    see module docstring), but this is kept identical to gen_commands.py /
    gen_commands_osf.py so _log_stem() behaves the same way if/when it's
    added.
    """
...
def build_train_cmd(exp: dict, registry: dict, context: str,
    ...
    logs_dir = registry.get("logs_dir", str(Path(__file__).parent.parent / "logs_physioomni"))
...
def build_analyze_cmd(exp: dict, registry: dict, plot: bool = False,
    ...
    # lives in physioomni_env for this registry (physioomni_env has its own
    # sklearn install, same as osf_env/sleepfm_env do for their pipelines).
    python = registry.get("python_bin", "/home/boshra95/physioomni_env/bin/python")
...
def build_probe_batch_cmd(exp: dict, registry: dict,
    ...
    memory-bounded exp. NOTE: jobs/find_batch_size_physioomni_gpu.sh does not
    exist yet — no current PhysioOmni experiment uses batch_mode: memory_bounded.
...
def cmd_probe_batch(args, registry):
    ...
    print(f"# Run BEFORE 'gen_commands_physioomni.py train {args.exp_id}'{probed_note}")
    print()
...
def cmd_train(args, registry):
    ...
    print(f"# NOTE: wall-time estimates are placeholder (not GPU-calibrated for PhysioOmni yet)")
    print()
    for ctx in contexts:
...
def cmd_infer(args, registry):
    ...
    print(f"# Est. wall time: {wall}  Logs → {registry.get('logs_dir', 'logs_physioomni/')}")
    print()
...
def cmd_threshold_tuning(args, registry):
    ...
    if val_missing:
        ...
        print(f"#   Run val inference first (see gen_commands_physioomni.py infer {args.exp_id} --split val)")
    print()
    results_dir = Path(registry["results_dir"])
...
def cmd_runs(args, registry):
    logs_dir = Path(registry.get("logs_dir", str(Path(__file__).parent.parent / "logs_physioomni")))
...
def cmd_collect(args, registry):
    ...
    results_dir = Path(registry["results_dir"])
    cdir = collected_dir or str(results_dir / "collected")
    print("# Collect all PhysioOmni experiment results into training.csv and analysis.csv")
    print(f"# Output → {cdir}/")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate sbatch/python commands for PhysioOmni baseline experiments.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--registry", default=str(REGISTRY_PATH),
                        help="Path to v2_physioomni_registry.yaml")
