...
infer_mantis_subject_windows.py, but the LoRA-adapted backbone must run
live on raw signal per window instead of reading a precomputed .npy
embedding, since the fine-tuned weights change what the backbone produces.

Reuses (not duplicates) train_mantis_lora.py's model-construction code
(CombinedMantisLoRAModel via build_combined_lora_model) — same reasoning
as train_mantis_lora.py importing run_epoch/compute_metrics from
train_mantis_context_sweep.py: this logic is already tested, and
duplicating it here would risk architecture drift between train and
inference. Same resumable-inference design as OSF's/PhysioOmni's own
Stage 2 inference scripts (periodic time-based checkpointing, not just
per-epoch resume) — a timed-out multi-hour "all windows" pass must not
have to restart from item 0.

SEQ2LABEL ONLY — matches MantisRawEpochWindowDataset's current scope.

BATCH SIZE — deliberately does NOT reuse Stage 1's context-length-based
auto-scaling formula (_ref_bs/_ref_N, itself never GPU-verified for any of
the three models). That formula was calibrated for looking up cheap
precomputed embeddings; here every item still runs a full LoRA-adapted
backbone forward pass per raw epoch (chunked inside
CombinedMantisLoRAModel.forward via chunk_batch_size), so the compute/
memory profile is fundamentally different — same reasoning OSF's/
PhysioOmni's own Stage 2 inference scripts already used. Default here is a
fixed --batch-size, 32, a starting point NOT yet GPU-calibrated for
Mantis specifically — lower it if a longer context OOMs.
...
Usage:
    ...
    python scripts/infer_mantis_lora_subject_windows.py ... --no-all-windows
"""
...
def build_dataset(cfg: dict, split: str, context_length: str,
                  ...
                  limit: int = None) -> MantisRawEpochWindowDataset:
    """Build a MantisRawEpochWindowDataset, optionally overriding K_max to
    use all windows. Does not modify phase0_mantis_lora_config.yaml.

    limit (debug only, NOT present in Stage 1's inference script): caps the
    subject pool to the first N subjects of the split — Stage 2 runs a live
    LoRA-adapted backbone forward pass per window, so full-scope CPU
    debugging is impractically slow without restricting subject count."""
    if all_windows:
...
def get_subject_ids(ds: MantisRawEpochWindowDataset) -> list:
...
def run_inference(model: torch.nn.Module, ds, device: torch.device,
    ...
    partway through. See module docstring for the full design rationale."""
    n_total = len(ds)
    saved_logits  = None
    saved_targets = None
    n_done = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all-window Mantis-LoRA inference for subject-level aggregation."
    )
    ...
    parser.add_argument("--limit",      default=None, type=int,
                        help="DEBUG ONLY, not in Stage 1's inference script: cap the subject "
                             "pool to the first N subjects of the split.")
    ...
    print("=" * 68)
    print("Mantis baseline — Stage 2 (LoRA) — Subject-level inference")
    print("=" * 68)
