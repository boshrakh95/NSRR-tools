#!/usr/bin/env python3
"""
infer_mantis_subject_windows.py — Mantis baseline, Stage 1 Step 9
...
Forked from scripts/infer_physioomni_subject_windows.py (itself forked from
infer_osf_subject_windows.py) per docs/TSFM_MANTIS_IMPLEMENTATION_PLAN.md
§10 — backbone-agnostic except for the dataset import and the (dropped)
SleepFM modality-ablation flag; Mantis has no 4-modality-group structure to
ablate either, same as OSF's and PhysioOmni's own forks.

ONE DELIBERATE ADDITION vs the template (plan §4.2): TF32 is enabled at the
top of `main()`, same as train_mantis_context_sweep.py. Inference is
forward-only so the effect is smaller than training, but it's still real
matmul work (a full-night Transformer/LSTM forward over up to 480
timesteps), and §4.2 calls for TF32 on every Mantis script that touches a
GPU — no reason to be the one exception.

OPEN CAVEAT, CARRIED HONESTLY (plan §10): the batch-size auto-scaling
reference (`_ref_bs=64` at `_ref_N=480`) is copied unchanged from OSF's/
PhysioOmni's own scripts, not re-derived for Mantis's specific per-token
compute cost. All three share the same token unit (one row per 30-second
epoch), so the scaling *shape* transfers, but `_ref_bs=64` itself has never
been GPU-verified for any of the three models — this carries the same
open question forward a third time rather than pretending it's been solved.
...
Usage:
    ...
    python scripts/infer_mantis_subject_windows.py ... --split val

    # Use only K=5 windows (reproduces training eval exactly):
    python scripts/infer_mantis_subject_windows.py ... --no-all-windows

    # Point at the Pilot 1/2 population instead of the production
    # embedding_dir (before checklist 1.11's full extraction has run):
    python scripts/infer_mantis_subject_windows.py ... \\
...
def build_dataset(cfg: dict, split: str, context_length: str,
                  ...
                  datasets_filter: list, all_windows: bool) -> MantisContextWindowDataset:
    """Build a MantisContextWindowDataset, optionally overriding K_max to use all windows."""
    if all_windows:
        ...
        # phase0_mantis_config.yaml — only the in-memory cfg copy.
...
def get_subject_ids(ds: MantisContextWindowDataset) -> list:
...
def get_anchor_patch_ends(ds: MantisContextWindowDataset) -> np.ndarray:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all-window Mantis inference for subject-level aggregation."
    )
    ...
    parser.add_argument("--embedding-dir", default=None, dest="embedding_dir",
                        help="Override dataset.embedding_dir (e.g. Pilot 1/2 population "
                             "before checklist 1.11's full extraction has run).")
    ...
    print("=" * 68)
    print("Mantis baseline — Subject-level inference")
    print("=" * 68)
    ...
    for ctx in contexts:
        ...
        try:
            ...
            # Same formula as infer_osf_subject_windows.py/
            # infer_physioomni_subject_windows.py, kept unchanged (not
            # re-derived) rather than blindly copied: Mantis's dataset uses
            # the exact same token unit as both — one row per 30-second
            # epoch, PATCHES_PER_EPOCH=1 (see
            # mantis_context_window_dataset.py's module docstring) — so
            # 240m is 480 epochs in all three, same as OSF's/PhysioOmni's
            # own _ref_N. Per-token embedding width here is 3072 (6 channel
            # slots x 512), wider than PhysioOmni's 500 and OSF's 1536, so
            # if this reference is miscalibrated it is more likely to be
            # too generous, not too conservative — worth watching, not yet
            # empirically re-verified on a real GPU for Mantis specifically
            # (same open caveat OSF's and PhysioOmni's own scripts carry).
            N_patches  = ds.N                  # epochs per window at this context
            _ref_bs    = 64                    # starting assumption, not yet GPU-verified for Mantis
            _ref_N     = 480                   # 240m in 30s-epoch units — same unit as OSF/PhysioOmni
