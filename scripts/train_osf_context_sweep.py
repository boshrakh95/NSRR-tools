#!/usr/bin/env python3
"""
train_osf_context_sweep.py — OSF baseline, Stage 1 Step 4

Sweeps over context lengths defined in phase0_osf_config.yaml, training a
lightweight sequence head on frozen OSF embeddings. Forked from
train_context_sweep.py per docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §3.3 —
per the Code Reuse Assessment, this script is backbone-agnostic except for
one artifact (the SleepFM 4-modality `--zero-modalities` ablation flag),
which is dropped entirely here since OSF has no modality-group structure
to ablate. Everything else — checkpoint/resume, early stopping, overfit
phase, snapshots, class-weight/sampler handling, metrics — carries over
unchanged, since it's head/optimizer-level, not backbone-level. Training
parameters (epochs, lr, optimizer, scheduler, early stopping, K-sampling
strategy, etc.) are intentionally kept identical to the SleepFM pipeline
for a fair comparison — see docs/TSFM_OSF_IMPLEMENTATION_PLAN.md's Key
Decisions table for the (very short) list of what's OSF-specific.

For each context length L:
  1. Build DataLoaders from OSFContextWindowDataset
...
────────────────────
  ...
  full_night: produces variable-length tensors (T varies per subject/anchor).
    DataLoader uses OSFContextWindowDataset.collate_fn to pad within each batch.
...
USAGE
─────
  python scripts/train_osf_context_sweep.py --config configs/phase0_osf_config.yaml
  ...
  python scripts/train_osf_context_sweep.py \\
      --config configs/phase0_osf_config.yaml --context 30s 10m
...
def train_one_context(
    ...
    wandb_project:   str  = "nsrr-phase0-osf",
    ...
    def make_ds(split):
        with warnings.catch_warnings():
            ...
            return OSFContextWindowDataset(
                cfg=cfg,
                split=split,
    ...
    # ── DataLoaders ────────────────────────────────────────────────────────
    collate     = OSFContextWindowDataset.collate_fn if is_full_night else None
...
def main():
    parser = argparse.ArgumentParser(description="OSF baseline context-length sweep")
    ...
    parser.add_argument("--wandb-project", default="nsrr-phase0-osf", dest="wandb_project",
                        help="W&B project name (default: nsrr-phase0-osf — deliberately "
                             "separate from SleepFM's nsrr-phase0 project)")
