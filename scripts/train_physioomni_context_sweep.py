#!/usr/bin/env python3
"""
train_physioomni_context_sweep.py — PhysioOmni baseline, Stage 1 Step 4

Sweeps over context lengths defined in phase0_physioomni_config.yaml,
training a lightweight sequence head on frozen PhysioOmni embeddings.
Forked from train_osf_context_sweep.py per
docs/TSFM_PHYSIOOMNI_IMPLEMENTATION_PLAN.md §10 — deliberately kept with
the SAME clean function boundaries (`run_epoch`, `compute_metrics`,
`compute_monitor_metric`, `append_to_summary`, `_classify_failure`)
OSF's own script already has, so a future PhysioOmni Stage 2 (LoRA) script
can import them directly the way `train_osf_lora.py` imports from
`train_osf_context_sweep.py` — this costs nothing now and avoids a real
refactor later (§10, §15.2's lesson from OSF's own Stage 2 build).

Nothing here is backbone-specific except the dataset-class import — per
the Code Reuse Assessment (`CLAUDE.md`), this script only ever calls
`model(x, mask)` without knowing what's inside `model`, so everything
below works unmodified once `PhysioOmniContextWindowDataset` exists.
PhysioOmni has no 4-modality-*group* structure to ablate (its "4
modalities" — EEG/EOG/ECG/EMG — are already the finest granularity), so
there is no `--zero-modalities` flag here, same as OSF's own fork.

For each context length L:
  1. Build DataLoaders from PhysioOmniContextWindowDataset
...
────────────────────
  ...
  full_night: produces variable-length tensors (T varies per subject/anchor).
    DataLoader uses PhysioOmniContextWindowDataset.collate_fn to pad within
    each batch. Transformer head is automatically skipped for full_night
    (O(N²) memory). full_night may have fewer training examples (K=1 window
    per subject for seq2label) — consider --full-night-epochs to run for
    more epochs.

  NOTE (docs/TSFM_PHYSIOOMNI_IMPLEMENTATION_PLAN.md §19): PhysioOmni's own
  encoder could architecturally support somewhat longer native windows than
  OSF (up to ~8.5 min for EEG at its own reference resample rate), but this
  script still trains a sequence head across ALL context lengths — even
  10m already exceeds every modality's native ceiling, so Plan B applies
  at every point in the sweep except the trivial 30s one. See plan §19 for
  the full reasoning; nothing about that changes what this script does.

USAGE
─────
  python scripts/train_physioomni_context_sweep.py --config configs/phase0_physioomni_config.yaml
  ...
  python scripts/train_physioomni_context_sweep.py \\
      --config configs/phase0_physioomni_config.yaml --context 30s 10m
...
def train_one_context(
    ...
    wandb_project:   str  = "nsrr-phase0-physioomni",
    ...
    def make_ds(split):
        with warnings.catch_warnings():
            ...
            return PhysioOmniContextWindowDataset(
                cfg=cfg,
                split=split,
    ...
    # ── DataLoaders ────────────────────────────────────────────────────────
    collate     = PhysioOmniContextWindowDataset.collate_fn if is_full_night else None
...
def main():
    parser = argparse.ArgumentParser(description="PhysioOmni baseline context-length sweep")
    ...
    parser.add_argument("--wandb-project", default="nsrr-phase0-physioomni", dest="wandb_project",
                        help="W&B project name (default: nsrr-phase0-physioomni — deliberately "
                             "separate from SleepFM's nsrr-phase0 and OSF's nsrr-phase0-osf projects)")
