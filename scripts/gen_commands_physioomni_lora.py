#!/usr/bin/env python3
"""
Experiment command generator for PhysioOmni Stage 2 (LoRA) experiments
(v2_physioomni_lora_registry.yaml).

Forked from gen_commands_osf_lora.py — see
docs/TSFM_PHYSIOOMNI_IMPLEMENTATION_PLAN.md §15/checklist 2.5. Same core
pipeline, status tracking, and command-generation approach as OSF's Stage
2 generator; a separate script rather than a gen_commands_physioomni.py
retrofit, same reasoning as every other gen_commands_*.py in this project
(no backbone/stage hook in the registry schema).
Differences from gen_commands_physioomni.py (Stage 1), all deliberate —
same shape as gen_commands_osf_lora.py's own differences from
gen_commands_osf.py:
  - Targets experiments/v2_physioomni_lora_registry.yaml, not
    v2_physioomni_registry.yaml — 8 entries (4 tasks x lstm/transformer),
    not 12: mean_pool is deferred for this first Stage 2 pass (plan
    §15.10, same scoping decision OSF's own Stage 2 made).
  - Targets jobs/{train,infer}_physioomni_lora*_gpu.sh (Fir only).
  - HAS gradient-accumulation machinery (`resolve_batch_accum`,
    `ACCUM_STEPS`), same as gen_commands_physioomni.py — effective_batch=32,
    same as Stage 1/SleepFM/OSF, required for comparable training
    dynamics, not just comparable results. No `memory_bounded` batch_mode
    branch — dropped along with probe-batch below (no probing infra
    exists yet).
  - NO `probe-batch` subcommand — no PhysioOmni-LoRA experiment uses a
    memory_bounded batch mode and no find_batch_size_physioomni_lora_gpu.sh
    exists; unlike gen_commands_physioomni.py this isn't even kept for
    schema parity, since Stage 2 has no batch_mode concept in its registry
    at all.
  - NO TASK_TYPE env var in generated train/infer commands —
    train_physioomni_lora.py / infer_physioomni_lora_subject_windows.py
    have no --task-type flag (seq2label-only for now).
  - Adds an optional --stage1-checkpoint override for `train` (normally
    omitted — train_physioomni_lora.py auto-detects the matching Stage 1
    checkpoint by task/head/context when none is given).
  - Wall-time lookup tables are placeholder copies of Stage 1's OWN
    (already-placeholder) PhysioOmni tables, scaled up by a flat, clearly-
    unverified multiplier — Stage 2 runs live encoder forward+backward
    passes per raw epoch every step (not a cheap cached-embedding lookup
    like Stage 1), so it is qualitatively far more expensive per step, but
    no real GPU number exists yet to calibrate against (checklist 2.6).
    Auto-requeue means an underestimate just costs one resubmission, not
    lost work.
  - `analyze`/`build-heatmap`/`collect`/`threshold-tuning` subcommands are
    ...
    just pointed at Stage 2's own results_dir.

── Core pipeline ─────────────────────────────────────────────────────────────
  python scripts/gen_commands_physioomni_lora.py list [--tier 1|2]
      List all PhysioOmni-LoRA experiments with status (pending/trained/inferred/analyzed).

  python scripts/gen_commands_physioomni_lora.py train <exp_id> [--context 30s 10m ...]
      Print sbatch command(s) for LoRA training. One job per context.

  python scripts/gen_commands_physioomni_lora.py infer <exp_id> [--split test|val]
      Print the sbatch command for LoRA inference (auto-discovers trained contexts).

  python scripts/gen_commands_physioomni_lora.py analyze <exp_id> [--plot] [--k-dense] [--bootstrap N]
      Print the python command for window analysis (analyze_windows.py —
      reused unmodified; reads the PhysioOmni-LoRA results dir).

  python scripts/gen_commands_physioomni_lora.py build-heatmap <exp_id> [--split test]
      Print the build_heatmap_df.py command (iso-compute heatmap DataFrame;
      reused unmodified). Run analyze --k-dense first.

  python scripts/gen_commands_physioomni_lora.py collect [<exp_id> ...]
      Print collect_results_v2.py command to gather all PhysioOmni-LoRA results into
      training.csv and analysis.csv (reused unmodified).

  python scripts/gen_commands_physioomni_lora.py threshold-tuning <exp_id>
      Print apply_threshold_tuning.py command for a binary experiment
      (reused unmodified).

  python scripts/gen_commands_physioomni_lora.py status [<exp_id>]
      Show detailed file-level status for one or all experiments.

  python scripts/gen_commands_physioomni_lora.py runs [<exp_id>]
      Show job run history (from logs_physioomni/status/*.jsonl tracking files).

Examples:
  python scripts/gen_commands_physioomni_lora.py list --tier 1
  python scripts/gen_commands_physioomni_lora.py train sex_binary_lstm
  python scripts/gen_commands_physioomni_lora.py train sex_binary_lstm --context 30s
  python scripts/gen_commands_physioomni_lora.py infer sex_binary_lstm
  python scripts/gen_commands_physioomni_lora.py analyze sex_binary_lstm --plot
  python scripts/gen_commands_physioomni_lora.py collect sex_binary_lstm bmi_binary_transformer
  python scripts/gen_commands_physioomni_lora.py status
  python scripts/gen_commands_physioomni_lora.py runs sex_binary_lstm
"""
...
REGISTRY_PATH = Path(__file__).parent.parent / "experiments" / "v2_physioomni_lora_registry.yaml"
JOBS_DIR = Path(__file__).parent.parent / "jobs"

_TRAIN_SCRIPT = "train_physioomni_lora_gpu.sh"
_INFER_SCRIPT = "infer_physioomni_lora_subject_windows_gpu.sh"

# ── Wall-time lookup tables ────────────────────────────────────────────────────
# NOT YET CALIBRATED FOR PHYSIOOMNI-LORA — placeholder: Stage 1's own
# (already-placeholder, see gen_commands_physioomni.py) PhysioOmni tables,
# scaled by a flat 6x multiplier (same starting multiplier OSF's own Stage
# 2 generator used). The 6x is an unverified, qualitative guess (live
# encoder forward+backward per raw epoch every step vs. Stage 1's cached-
# embedding lookup), not a measurement — plausibly an underestimate here
# given up to 4 encoder forward passes per epoch instead of OSF's 1 (plan
# §15.9) — revisit after checklist 2.6's real GPU pilot.
...
def _log_stem(exp: dict, step: str, context: str = "", split: str = "") -> str:
    ...
    No arch_tag here (unlike gen_commands_physioomni.py) — no PhysioOmni-
    LoRA experiment is seq2seq, and train_physioomni_lora.py has no
    seq2seq_padding_policy concept at all yet, so there's nothing to
    disambiguate."""
    tag = exp.get("run_tag", "")
...
def build_analyze_cmd(exp: dict, registry: dict, plot: bool = False,
    ...
    # agnostic already by gen_commands_osf.py/gen_commands_physioomni.py.
...
def cmd_threshold_tuning(args, registry):
    ...
    if val_missing:
        ...
        print(f"#   Run val inference first (see gen_commands_physioomni_lora.py infer {args.exp_id} --split val)")
    print()
    results_dir = Path(registry["results_dir"])
...
# repo mirrors are round-tagged (results_dir.name, e.g. phase0_physioomni_lora) so
...
def cmd_collect(args, registry):
    ...
    results_dir = Path(registry["results_dir"])
    cdir = collected_dir or str(results_dir / "collected")
    print("# Collect all PhysioOmni-LoRA experiment results into training.csv and analysis.csv")
    print(f"# Output → {cdir}/")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate sbatch/python commands for PhysioOmni Stage 2 (LoRA) experiments.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--registry", default=str(REGISTRY_PATH),
                        help="Path to v2_physioomni_lora_registry.yaml")
    ...
    p_train.add_argument("--stage1-checkpoint", default=None, dest="stage1_checkpoint",
                         help="Override the Stage 1 checkpoint path for ALL generated contexts "
                              "(normally omitted — train_physioomni_lora.py auto-detects the matching "
