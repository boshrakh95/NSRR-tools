...
_TRAIN_SCRIPT_BY_CLUSTER = {
    "fir": "train_mantis_lora_gpu.sh",
    "nibi": "train_mantis_lora_gpu_nibi.sh",
}
_INFER_SCRIPT_BY_CLUSTER = {
    "fir": "infer_mantis_lora_subject_windows_gpu.sh",
    "nibi": "infer_mantis_lora_subject_windows_gpu_nibi.sh",
}
...
def build_train_cmd(exp: dict, registry: dict, context: str,
                    ...
                    stage1_checkpoint: str = None, cluster: str = "fir") -> str:
...
def cmd_train(args, registry):
    ...
    print(f"# LoRA training commands for: {args.exp_id}  (cluster: {args.cluster})")
...
def cmd_infer(args, registry):
    ...
    print(f"# LoRA inference command for: {args.exp_id}  split={split}  (cluster: {args.cluster})")
