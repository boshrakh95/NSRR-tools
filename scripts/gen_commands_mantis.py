#!/usr/bin/env python3
"""
Experiment command generator for Mantis baseline experiments (v2_mantis_registry.yaml).

Forked from gen_commands_physioomni.py (itself forked from gen_commands_osf.py)
per docs/TSFM_MANTIS_IMPLEMENTATION_PLAN.md §11 — same core pipeline, status
tracking, and command-generation approach as the SleepFM/OSF/PhysioOmni
generators; a separate script rather than a gen_commands.py retrofit per the
"Code reuse assessment" decision in CLAUDE.md (no backbone hook in the
original registry schema/wall-time tables).

Differences from gen_commands_physioomni.py, all deliberate:
  - Targets experiments/v2_mantis_registry.yaml, not
    v2_physioomni_registry.yaml — 5 Tier-1 tasks (sex, sleep efficiency,
    BMI, age, apnea), not 4: apnea_binary IS included. Mantis is a single
    channel-agnostic encoder (every channel, including RESP, goes through
    the same frozen weights, plan §2.2) — unlike PhysioOmni, it has no
    missing-modality gap, making it the only one of the three baselines
    directly comparable to OSF on apnea.
  - Targets jobs/{train,infer}_mantis_*_gpu.sh (Fir only — no rorqual
    variant exists yet for Mantis, same as OSF/PhysioOmni).
  - No --zero-modalities / ZERO_MODALITIES support (Mantis has no
    4-modality-group structure to ablate — its 6 "subtokens" are already
    the finest granularity, the 6 channel slots — dropped from
    train_mantis_context_sweep.py / infer_mantis_subject_windows.py
    themselves, same as OSF's/PhysioOmni's own forks).
  - `--registry` also accepts experiments/v2_mantis_plus_registry.yaml for
    the MantisPlus ablation (plan §11/§5.1) — same schema, different
    config/results/logs paths, selected via --registry, not a separate script.
  - python_bin fallback defaults to mantis_env, not physioomni_env/osf_env.
  - Figure/table subcommands (iso-plots, saturation, scaling-laws,
    ...
    are NOT ported — same reasoning as gen_commands_osf.py/
    gen_commands_physioomni.py: SleepFM's own plot_*.py/make_table*.py
    scripts are already superseded by notebooks
    (results/paper_figures/notebooks_npj/) for the current paper, so
    there's no reason to build a second, parallel figure-generation code
    path here either. Once Mantis results exist, feed them into notebooks
    the same way SleepFM's/OSF's/PhysioOmni's are fed in.
  - Wall-time lookup tables are seeded from the Pilot-3 GPU measurement
    (plan §13.3 — real single-subject-forward timing, not a placeholder
    copy of OSF's/PhysioOmni's numbers, which have a different compute
    profile: Mantis is ONE encoder run over 6 channels batched together,
    ~19x the per-token FLOPs of OSF/PhysioOmni per plan §4.8). Still only
    calibrated from that one measurement, not a full GPU sweep — marked
    accordingly below, revisit after checklist 1.12's real sweep.

── Core pipeline ─────────────────────────────────────────────────────────────
  python scripts/gen_commands_mantis.py list [--tier 1|2]
      List all Mantis experiments with status (pending/trained/inferred/analyzed).

  python scripts/gen_commands_mantis.py probe-batch <exp_id> [--starting-batch-size 256]
      Print sbatch command to probe the max GPU batch size (memory_bounded
      experiments only). No Mantis experiment currently uses batch_mode:
      memory_bounded, and jobs/find_batch_size_mantis_gpu.sh does not exist yet
      — this subcommand is kept for schema parity / future use only.

  python scripts/gen_commands_mantis.py train <exp_id> [--context 30s 10m ...]
      Print sbatch command(s) for training. One job per context.

  python scripts/gen_commands_mantis.py infer <exp_id> [--split test|val]
      Print the sbatch command for inference (auto-discovers trained contexts).

  python scripts/gen_commands_mantis.py analyze <exp_id> [--plot] [--k-dense] [--bootstrap N]
      Print the python command for window analysis (analyze_windows.py —
      backbone-agnostic, reused unmodified; reads the Mantis results dir).

  python scripts/gen_commands_mantis.py build-heatmap <exp_id> [--split test]
      Print the build_heatmap_df.py command (iso-compute heatmap DataFrame;
      reused unmodified). Run analyze --k-dense first.

  python scripts/gen_commands_mantis.py collect [<exp_id> ...]
      Print collect_results_v2.py command to gather all Mantis results
      into training.csv and analysis.csv (reused unmodified).

  python scripts/gen_commands_mantis.py threshold-tuning <exp_id>
      Print apply_threshold_tuning.py command for a binary experiment
      (reused unmodified).

  python scripts/gen_commands_mantis.py status [<exp_id>]
      Show detailed file-level status for one or all experiments.

  python scripts/gen_commands_mantis.py runs [<exp_id>]
      Show job run history (from logs_mantis/status/*.jsonl tracking files).

Examples:
  python scripts/gen_commands_mantis.py list --tier 1
  python scripts/gen_commands_mantis.py train sex_binary_lstm
  python scripts/gen_commands_mantis.py train sex_binary_lstm --context 30s
  python scripts/gen_commands_mantis.py infer sex_binary_lstm
  python scripts/gen_commands_mantis.py analyze sex_binary_lstm --plot
  python scripts/gen_commands_mantis.py collect sex_binary_lstm bmi_binary_transformer
  python scripts/gen_commands_mantis.py status
  python scripts/gen_commands_mantis.py runs sex_binary_lstm

  # MantisPlus ablation (once its registry/config exist, plan §5.1/§11):
  python scripts/gen_commands_mantis.py --registry experiments/v2_mantis_plus_registry.yaml list
"""
...
# Seeded from Pilot 3's real GPU measurement (plan §13.3: single-subject
# full-epoch forward pass, Option D, 1g.10gb MIG slice) — a real data point,
# not a blind copy of OSF's/PhysioOmni's numbers, which have a different
# compute profile (Mantis runs ONE encoder over 6 batched channels, ~19x the
# per-token FLOPs of either, plan §4.8). Still calibrated from exactly one
# measurement, not a full GPU training sweep — these are starting points to
# submit against, not settled numbers. Revisit after checklist 1.12's real
# sweep. Auto-requeue on timeout means an underestimate just costs one extra
# resubmission, not lost work (resume.pt is saved after every epoch).
#
# NOTE: unlike OSF/PhysioOmni, Stage 1 training here never touches the Mantis
# backbone (embeddings are pre-extracted, checklist 1.8's module docstring) —
# so these hours are for the lightweight sequence-head training loop only,
# not backbone compute. They should end up closer to (or below) PhysioOmni's
# own Stage-1 numbers, not scaled up by Mantis's heavier per-token FLOPs —
# that heavier cost lives entirely in extract_mantis_embeddings.py and (later)
# train_mantis_lora.py, not here.
_TRAIN_HOURS = {
...
# Also NOT YET FULLY CALIBRATED — same caveat as above.
...
def _cfg_arch_tag(exp: dict, registry: dict) -> str:
    ...
    No Mantis registry entry is seq2seq (sleep_staging is out of scope for
    all three baselines, plan §5.8), but this is kept identical to
    gen_commands.py / gen_commands_osf.py / gen_commands_physioomni.py so
    _log_stem() behaves the same way if that scope ever changes.
    """
...
def build_analyze_cmd(exp: dict, registry: dict, plot: bool = False,
    ...
    # lives in mantis_env for this registry (mantis_env has its own sklearn
    # install, same as physioomni_env/osf_env/sleepfm_env do for theirs).
...
def build_probe_batch_cmd(exp: dict, registry: dict,
    ...
    memory-bounded exp. NOTE: jobs/find_batch_size_mantis_gpu.sh does not
    exist yet — no current Mantis experiment uses batch_mode: memory_bounded.
...
def cmd_probe_batch(args, registry):
    ...
    print(f"# Run BEFORE 'gen_commands_mantis.py train {args.exp_id}'{probed_note}")
    print()
...
def cmd_train(args, registry):
    ...
    print(f"# Logs → {registry.get('logs_dir', 'logs_mantis/')}")
    print(f"# NOTE: wall-time estimates seeded from Pilot 3's single-point GPU "
          f"measurement (plan §13.3), not a full Mantis sweep yet")
    print()
    for ctx in contexts:
...
def cmd_threshold_tuning(args, registry):
    ...
    if val_missing:
        ...
        print(f"#   Run val inference first (see gen_commands_mantis.py infer {args.exp_id} --split val)")
    print()
    results_dir = Path(registry["results_dir"])
...
def cmd_runs(args, registry):
    logs_dir = Path(registry.get("logs_dir", str(Path(__file__).parent.parent / "logs_mantis")))
...
# repo mirrors are round-tagged (results_dir.name, e.g. phase0_mantis) so
...
def cmd_collect(args, registry):
    ...
    results_dir = Path(registry["results_dir"])
    cdir = collected_dir or str(results_dir / "collected")
    print("# Collect all Mantis experiment results into training.csv and analysis.csv")
    print(f"# Output → {cdir}/")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate sbatch/python commands for Mantis baseline experiments.",
    ...
    p_tt.add_argument("exp_id", help="Experiment ID, e.g. sex_binary_lstm")
