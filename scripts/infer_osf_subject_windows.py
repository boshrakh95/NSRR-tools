#!/usr/bin/env python3
"""
infer_osf_subject_windows.py — OSF baseline, Stage 1 Step 5
...
Forked from infer_subject_windows.py per
docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §3.3 (Appendix) — backbone-agnostic
except for the dataset import and the (dropped) SleepFM modality-ablation
flag; see the module docstring notes below for the one deliberate
OSF-specific recalibration (batch-size auto-scaling reference point).
...
Usage:
    ...
    python scripts/infer_osf_subject_windows.py ... --split val

    # Use only K=5 windows (reproduces training eval exactly):
    python scripts/infer_osf_subject_windows.py ... --no-all-windows
"""
...
from nsrr_tools.datasets.osf_context_window_dataset import (
...
def build_dataset(cfg: dict, split: str, context_length: str,
                  ...
                  datasets_filter: list, all_windows: bool) -> OSFContextWindowDataset:
    """Build an OSFContextWindowDataset, optionally overriding K_max to use all windows."""
    if all_windows:
        ...
        # phase0_osf_config.yaml — only the in-memory cfg copy.
        cfg["dataset"]["windows_per_subject"] = 99_999

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        ds = OSFContextWindowDataset(
            cfg=cfg,
            split=split,
...
def get_subject_ids(ds: OSFContextWindowDataset) -> list:
...
def get_anchor_patch_ends(ds: OSFContextWindowDataset) -> np.ndarray:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all-window OSF inference for subject-level aggregation."
    )
    ...
    parser.add_argument("--batch-size", default=512, type=int, dest="batch_size",
                        help="Upper-bound batch size (default 512), auto-scaled down for "
                             ...
                             "for how the reference point differs from SleepFM's.")
    ...
    print("=" * 68)
    print("OSF baseline — Subject-level inference")
    print("=" * 68)
    ...
    for ctx in contexts:
        ...
        try:
            ...
            # Same formula/intent as infer_subject_windows.py, but the reference
            # point is recalibrated to OSF's own context ladder rather than
            # SleepFM's — see docs/TSFM_OSF_IMPLEMENTATION_PLAN.md's Key
            # Decisions table. SleepFM's original reference was (_ref_bs=64,
            # _ref_N=2880), i.e. "training used batch=32 at 240m = 2880
            # 5s-patches". OSF's 240m is 480 30s-epochs, not 2880 — reusing
            # SleepFM's literal 2880 here would be dimensionally wrong (OSF's
            # longest context would look like it needs 6x less scaling-down
            # than it should relative to its own token count). _ref_bs=64 is
            # kept as a starting assumption (not re-derived — OSF's per-token
            # embedding is 3x wider than SleepFM's but its longest context has
            # 6x fewer tokens, so the two effects partially offset; this has
            # NOT been empirically verified on a real GPU yet, same caveat as
            # the original script's own comment below).
            N_patches  = ds.N                  # epochs per window at this context
            _ref_bs    = 64                    # starting assumption, not yet GPU-verified for OSF
            _ref_N     = 480                   # OSF's 240m in its own units (was SleepFM's 2880)
