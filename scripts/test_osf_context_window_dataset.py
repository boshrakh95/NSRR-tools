#!/usr/bin/env python3
"""
test_osf_context_window_dataset.py — smoke-test for OSFContextWindowDataset.

Forked from test_context_window_dataset.py per
docs/TSFM_OSF_IMPLEMENTATION_PLAN.md §12 item 2. Only required edit: the
flat-dim assertion (512 -> 1536, since OSF stacks CLS + mean-pooled patches
at 768-dim each, vs SleepFM's 4 modalities at 128-dim each).
...
def test_one(cfg, task, task_type, context, datasets_filter):
    ...
    for split in ("train", "val", "test"):
        ...
        assert x.shape[-1] == 1536,       f"Expected flat dim 1536 (OSF: 2*768), got {x.shape[-1]}"
...
def main():
    ...
    print("\n\nAll OSFContextWindowDataset smoke-tests PASSED.")
