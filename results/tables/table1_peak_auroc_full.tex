\begin{table}[ht]
\centering
\caption{Peak test AUROC for each seq2label task and head (channel: full). Best\_L: context length at which AUROC is highest. K=5: deployment scenario (K windows per subject). K=all: full-night ceiling.}
\label{tab:peak_auroc_full}
